...
export function createUI(tunnelName, role) {
    ...
    let disposed = false;
    let iceState = 'new';
    let inactivityRemainingMs = null;
    ...
    function moveTo(x, y) { process.stdout.write(`\x1b[${y + 1};${x + 1}H`); }
    function padRight(s, w) { return s + ' '.repeat(Math.max(0, w - stripAnsi(s).length)); }
    ...
    function render() {
        ...
        moveTo(2, 1);
        process.stdout.write(padRight(`${brand}   ${tunnel}   ${roleStr}   ${ind.dot} ${ind.label}   ${countdown}`, totalW - 4));
        moveTo(2, 2);
    ...
    return {
        ...
        setIceState(state) { iceState = state; render(); },
        resetInactivity(totalMs) {
