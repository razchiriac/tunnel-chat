...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && req.url?.startsWith('/auth/key')) {
    ...
    try {
      ...
      // Pass 1: return the first valid existing key across customers
      ...
      // Pass 2: if any customer has an active subscription but no valid key, retro-provision
      for (const c of customers.data) {
        try {
          ...
          console.error('[billing] subscription check failed during retro-provision (multi-customer):', (e as Error).message);
        }
      }
